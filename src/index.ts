...
const argv = cli({
    ...
    flags: {
        ...
        root: {
            type: String,
            alias: "r",
            description: "Custom name to use for the root namespace, it defaults to a kebab case version of <interface> prefixed by an @.",
        },
    },
    help: {
        examples: [
...
const additions = argv._.additions.map((input) => {
    ...
    if (split.length !== 3)
        throw Error(`Invalid addition '${input}'`);
...
if (argv.flags.notice)
    out.addStatements(`// ${argv.flags.notice}`);
